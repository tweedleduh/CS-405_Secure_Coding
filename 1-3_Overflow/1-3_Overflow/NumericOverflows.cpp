...
{
    // Capture the start variable
    T result = start;
    // Code that causes the addition
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // Detect overflow by checking against max numeric limit for type
        if (result >= 0 && (increment > (std::numeric_limits<T>::max() - result))) 
        {
            // Raise an error for the overflow
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // Detect underflow by checking against max numeric limit for type
        if (decrement < (std::numeric_limits<T>::min() - result))
        {
